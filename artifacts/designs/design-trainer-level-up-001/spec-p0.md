...
## E. Skill Rank Allocation Step

### New File: `app/components/levelup/LevelUpSkillSection.vue`

Allocate skill rank-ups gained from leveling. Each level grants 1 skill rank-up. The trainer picks which skill to raise by one step.
...
interface Props {
  ...
  targetLevel: number
  /** Skill rank caps unlocked in this advancement (e.g., 'Adept' at level 2) */
...
```
┌─────────────────────────────────────────┐
│ Skill Rank-Ups (+3 ranks)              │
│                                         │
│ Ranks Remaining: 1 / 3                 │
│                                         │
│ New Skill Cap: Expert (unlocked at L6) │
│                                         │
│ ┌─ Body ─────────────────────────────┐ │
│ │ Acrobatics  [Untrained]      [+]   │ │
│ │ Athletics   [Novice]         [+]   │ │
│ │ Combat      [Pathetic] → Untrained │ │
│ │ Intimidate  [Untrained]      [+]   │ │
│ │ Stealth     [Untrained]      [+]   │ │
│ │ Survival    [Adept]     (capped)   │ │
│ └────────────────────────────────────┘ │
│ ┌─ Mind ─────────────────────────────┐ │
│ │ ...                                 │ │
│ └────────────────────────────────────┘ │
...
- Skills are displayed grouped by category (Body/Mind/Spirit), matching `SkillBackgroundSection` and `EdgeSelectionSection` patterns
- Each skill shows current rank and a [+] button to select it for rank-up
- Selected skills appear in a "Chosen" list at the bottom with an [x] to remove
- **Skill rank cap enforcement:** Cannot raise a skill above the cap for the **target** level (not the starting level). If leveling from 5 to 7, the cap is based on level 7 (Expert since level 6 was crossed)
- **Pathetic skills CAN be raised during level-up** (not during creation per decree-027). The Pathetic-to-Untrained progression via Skill Edge is the standard post-creation mechanic (PTU p. 41). The regular skill rank-up can also raise Pathetic skills
- A skill cannot be raised above its cap. If the target level is 5, the cap is Adept. A skill at Adept cannot be raised further
- If a skill is chosen multiple times (across multiple level-ups in a jump), each selection raises it one step
- The computed "effective rank" accounts for all pending rank-ups to prevent double-raising above cap

#### Effective Rank Computation

When the GM selects a skill for rank-up, the component must track the "effective" rank (current rank + pending rank-ups from this session) to enforce caps:

```typescript
function getEffectiveRank(skill: PtuSkillName): SkillRank {
  const baseRank = props.currentSkills[skill]
  const pendingRankUps = props.skillChoices.filter(s => s === skill).length
...
function isSkillSelectable(skill: PtuSkillName): boolean {
  const effectiveRank = getEffectiveRank(skill)
  return !isSkillRankAboveCap(getNextRank(effectiveRank), props.targetLevel)
}
```
