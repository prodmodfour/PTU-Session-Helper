---
last_updated: 2026-03-02T11:30:00
updated_by: slave-collector (plan-20260302-110035)
---
...
| bug-042 | P2 | **in-progress** | release-hold.post.ts creates duplicate turnOrder entry in Full Contact battles after hold-release. First fix BLOCKED (code-review-268). Rewritten by slave-1 (plan-20260302-110035): remove-before-insert approach (remove original entry, adjust currentTurnIndex if removal shifts it, then insert). Tests updated to match corrected logic. Needs re-review |
...
| feature-014 | P1 | **P1-APPROVED** | VTT Flanking Detection — P0 APPROVED. P1 APPROVED: code-review-272 **APPROVED** (2 MED: app-surface.md update, stale decree-need-039 comment) + rules-review-248 **APPROVED** (0 issues). Reviewed by slave-4 (plan-20260302-110035). MEDs to address before P2 | multi-phase |
| feature-016 | P2 | **P2-CHANGES_REQUIRED** | Priority / Interrupt / Attack of Opportunity System — P0 APPROVED. P1 APPROVED. P2 reviewed by slave-5 (plan-20260302-110035): code-review-273 **CHANGES_REQUIRED** (CRIT-001: canIntercept && vs || for Full Action check; HIGH-001: diagonal movement ignores decree-002; HIGH-002: distance calcs ignore multi-tile footprints; HIGH-003: out-of-turn.service.ts 1361 lines; MED-001/002/003) + rules-review-249 **CHANGES_REQUIRED** (CRIT-001: same canIntercept logic; HIGH-001: raw speed without modifiers; MED-001/002). Needs fix cycle | multi-phase |
...
| feature-020 | P2 | **P0-APPROVED** | Healing Item System — P0 implemented + fix cycle complete. Re-reviewed by slave-3 (plan-20260302-110035): code-review-271 **APPROVED** (0 issues, all 7 code-review-267 issues verified resolved) + rules-review-247 **APPROVED** (0 issues, all PTU mechanics still correct). P0 complete, ready for P1 | multi-phase |
| feature-023 | P2 | **P0-fix-cycle-done** | Player Capture & Healing Interfaces — P0 implemented + fix cycle complete by slave-2 (plan-20260302-110035): 10 commits — fixed CRIT-001 ball type mismatch (DEFAULT_BALL_TYPE constant), HIGH-001 ballType passthrough, extracted useSwitchModalState composable (gm/index.vue size cap), removed unused prop, added undo snapshots, replaced alert() with inline errors, updated app-surface.md, fixed capture rate display, normalized import paths. All code-review-270 + rules-review-246 issues addressed. Needs re-review | multi-phase |
...
**Current task:** Session 84 collection complete. 5 slaves merged (19 commits total). 2 dev slaves (bug-042 rewrite 3 commits, feature-023 fix cycle 10 commits). 3 reviewer slaves (feature-020 APPROVED, feature-014-p1 APPROVED, feature-016-p2 CHANGES_REQUIRED). Smoke test PASSED. No conflicts.
