---
last_updated: 2026-03-01T15:20:00
updated_by: slave-collector (plan-20260301-143720)
---
...
| ptu-rule-120 | P4 | **fix-cycle-done** | Equipment-granted Naturewalk auto-derived from equipped items. 8 commits. code-review-222 **CHANGES_REQUIRED** (H1: no unit tests). Fix cycle by slave-4 (plan-20260301-143720): 2 commits — comprehensive unit tests for getEquipmentGrantedCapabilities + equipment Naturewalk path (equipmentBonuses.test.ts + combatantCapabilities.test.ts). Needs re-review. rules-review-198 **APPROVED**. M1→ux-011, M2→ptu-rule-125, MED-01→ptu-rule-126 |
...
| feature-006 | P1 | **P2-needs-fix-cycle** | Pokemon Evolution System — P0 APPROVED. P1 APPROVED. P2 implemented by slave-4 (plan-20260301-135300). Reviewed by slave-5 (plan-20260301-143720): code-review-248 **CHANGES_REQUIRED** (C1: undo doesn't revert evolution notes, H1: non-atomic DB writes, H2: stone not restored on undo, H3: no GM override UI for missing stone, M1: app-surface.md, M2→ux-014, M3→ux-015). rules-review-224 **CHANGES_REQUIRED** (C1: seed parser missing "Learn" keyword — 7 Pokemon species lose move-based triggers, M1: notes not in snapshot, M2: stone not restored on undo). Needs fix cycle | multi-phase |
...
| feature-011 | P1 | **P2-needs-fix-cycle** | Pokemon Switching Workflow — P0 APPROVED. P1 APPROVED. P2 implemented by slave-5 (plan-20260301-135300). Reviewed by slave-6 (plan-20260301-143720): code-review-249 **CHANGES_REQUIRED** (CRIT-001: missing pokemon_recalled/pokemon_released WS handlers, H1: app-surface.md, H2: no turn validation on recall/release, M1→refactoring-112, M2: duplicate recall side-effects, M3: overlapping tokens fallback). rules-review-225 **APPROVED** (11 mechanics verified, M1→ptu-rule-130: fainted pair exemption edge case). Needs fix cycle | multi-phase |
...
| feature-009 | P1 | **P0-fix-cycle-done** | Trainer XP & Advancement Tracking — P0 implemented. Fix cycle by slave-2 (plan-20260301-143720): 6 commits — removed console.log (M3), null xpToNextLevel at max level (M1), extracted processXpAward helper (M2), resolved stale CharacterModal data with refresh emit (H1), updated app-surface.md (H2), ticket/design updates. All code-review-246 issues resolved. Needs re-review. rules-review-222 **APPROVED** | multi-phase |
| feature-013 | P1 | **P1-implemented** | Multi-Tile Token System — P0 APPROVED. P1 implemented by slave-3 (plan-20260301-143720): 7 commits — multi-cell A* pathfinding with tokenSize param (Section F), flood-fill movement range with gridBounds (Section G), getTerrainCostForFootprint utility (Section H), revealFootprintArea fog of war action (Section I), tokenSize in isValidMove + ghost footprint outline (Section J), unit tests for multi-cell pathfinding, ticket/design updates. Needs review | multi-phase |
...
| feature-016 | P2 | **P0-fix-cycle-done** | Priority / Interrupt / Attack of Opportunity System — P0 implemented. Fix cycle by slave-1 (plan-20260301-143720): 9 commits — validated reactor eligibility before accepting AoO (CRIT-001), validated triggerType against AOO_TRIGGER_MAP keys (H1), corrected AOO_STRUGGLE_ATTACK_DAMAGE_BASE to 11 (H2), added reactor eligibility check to client AoO preview (M1), auto-decline pending AoOs on target faint (M2), refreshed record in detect endpoint (M3), added pending actions cleanup (M4), updated app-surface.md (H3), ticket/design updates. All code-review-247 issues resolved. Needs re-review. rules-review-223 **APPROVED** | multi-phase |
...
**Current task:** Session 75 collection complete. 6 slaves merged (28 commits). 4 dev slaves (feature-016-p0-fix 9 commits, feature-009-p0-fix 6 commits, feature-013-p1 7 commits, ptu-rule-120-fix 2 commits). 2 reviewer slaves (feature-006-p2-review CHANGES_REQUIRED, feature-011-p2-review CHANGES_REQUIRED). Smoke test PASSED. 4 follow-up tickets filed (ux-014, ux-015, refactoring-112, ptu-rule-130).
