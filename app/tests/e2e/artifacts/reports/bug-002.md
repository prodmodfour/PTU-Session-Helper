...
- [x] Regression fixes (Senior Reviewer findings):
  - 71b6987 — `seed.ts:312`: replaced `startsWith` with word-boundary check (`name + ' '` prefix) to fix Kabuto/Paras/Porygon/Klink false positives
  - 4515dbb — `seed.ts:247`: broadened species name regex to include digits, periods, colons, Unicode quotes — fixes Porygon2, Mr. Mime, Mime Jr., Mr. Rime, Type: Null, Farfetch'd, Sirfetch'd
  - 7c49daf — `seed.ts:318`: Mega form uses `maxEvolutionStage` instead of hardcoded `3`
- [x] Verified after re-seed: Kabuto 1/2, Paras 1/2, Porygon 1/3, Klink 1/3, Porygon2 2/3, all previously-missing species present
