...
import { applyMovementModifiers, useGridMovement } from '~/composables/useGridMovement'
import type { Combatant, PokemonCapabilities, Pokemon } from '~/types'
import type { StatusCondition, StageModifiers } from '~/types/combat'

// Mock useRangeParser — useGridMovement depends on it
vi.mock('~/composables/useRangeParser', () => ({
  useRangeParser: () => ({
    calculatePathCost: vi.fn(),
    getMovementRangeCells: vi.fn(),
    getMovementRangeCellsWithAveraging: vi.fn(),
  }),
}))
...
/**
 * Build a Pokemon combatant with optional capabilities for Naturewalk tests.
 */
function makePokemonCombatantWithCaps(capabilities?: Partial<PokemonCapabilities>): Combatant {
...
describe('getTerrainCostForCombatant — Naturewalk slow bypass (PTU p.322)', () => {
  ...
  function createGridMovement(combatant: Combatant) {
    const tokens = ref([{ combatantId: combatant.id, position: { x: 0, y: 0 }, size: 1 }])
    return useGridMovement({
      tokens,
      getCombatant: (id: string) => id === combatant.id ? combatant : undefined,
    })
  }

  it('should return base cost (1) for slow cell when combatant has matching Naturewalk', () => {
    // Slow normal terrain at (2,0); combatant with Naturewalk (Forest)
    // Forest maps to 'normal' => slow flag bypassed => cost 1 (not 2)
    terrainStore.setTerrain(2, 0, 'normal', { rough: false, slow: true })

    const combatant = makePokemonCombatantWithCaps({ naturewalk: ['Forest'] })
    const { getTerrainCostForCombatant } = createGridMovement(combatant)

    expect(getTerrainCostForCombatant(2, 0, combatant.id)).toBe(1)
  })

  it('should return doubled cost (2) for slow cell when combatant has non-matching Naturewalk', () => {
    // Slow normal terrain at (2,0); combatant with Naturewalk (Ocean)
    // Ocean maps to 'water' only — does NOT match 'normal' => slow NOT bypassed => cost 2
    terrainStore.setTerrain(2, 0, 'normal', { rough: false, slow: true })

    const combatant = makePokemonCombatantWithCaps({ naturewalk: ['Ocean'] })
    const { getTerrainCostForCombatant } = createGridMovement(combatant)

    expect(getTerrainCostForCombatant(2, 0, combatant.id)).toBe(2)
  })

  it('should return doubled cost (2) for slow cell when combatant has no Naturewalk', () => {
    terrainStore.setTerrain(2, 0, 'normal', { rough: false, slow: true })

    const combatant = makePokemonCombatantWithCaps()
    const { getTerrainCostForCombatant } = createGridMovement(combatant)

    expect(getTerrainCostForCombatant(2, 0, combatant.id)).toBe(2)
  })

  it('should return Infinity for blocking terrain even with Naturewalk', () => {
    terrainStore.setTerrain(2, 0, 'blocking')

    const combatant = makePokemonCombatantWithCaps({
      naturewalk: ['Forest', 'Mountain', 'Cave'],
    })
    const { getTerrainCostForCombatant } = createGridMovement(combatant)

    expect(getTerrainCostForCombatant(2, 0, combatant.id)).toBe(Infinity)
  })

  it('should return Infinity for water terrain without swim even with Naturewalk (Ocean)', () => {
    // Naturewalk bypasses slow/rough flags, NOT the swim requirement
    // Water terrain requires swim capability to be passable
    terrainStore.setTerrain(2, 0, 'water', { rough: false, slow: true })
  ...
  it('should bypass slow on water terrain when combatant can swim AND has Naturewalk (Ocean)', () => {
    terrainStore.setTerrain(2, 0, 'water', { rough: false, slow: true })
    ...
    // Water base cost is 1, slow would double to 2, but Naturewalk bypasses slow
    expect(getTerrainCostForCombatant(2, 0, combatant.id)).toBe(1)
  })

  it('should return base cost for non-slow terrain regardless of Naturewalk', () => {
    // Normal terrain without slow flag — cost is 1 regardless
    terrainStore.setTerrain(2, 0, 'normal', { rough: false, slow: false })
  ...
  it('should bypass slow on earth terrain when combatant can burrow AND has Naturewalk (Cave)', () => {
    terrainStore.setTerrain(2, 0, 'earth', { rough: false, slow: true })
    ...
    // Earth with burrow capability: base cost 1 (from TERRAIN_COSTS lookup path)
    // Slow bypassed by Naturewalk => cost 1
    expect(getTerrainCostForCombatant(2, 0, combatant.id)).toBe(1)
  })
})
