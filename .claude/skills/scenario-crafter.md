...
description: Turns gameplay loops into concrete, testable scenarios with real Pokemon data, UI actions, and quantitative assertions. Use when gameplay loops are ready and need to be converted into test cases, or when the Orchestrator directs you to craft scenarios. Triggers on /craft-scenarios.
---
...
This skill is the second stage of the **Testing Loop** in the 9-skill PTU ecosystem.
...
1. **Look up base stats** — read species files from `books/markdown/pokedexes/gen<N>/<species>.md` (see lookup guide in `references/ptu-chapter-index.md`)
...
5. **Calculate all derived values** — HP, evasion, damage, etc. using formulas from `references/ptu-chapter-index.md`

### Step 3: Map to UI Actions

Translate game actions to specific UI interactions using `references/app-surface.md`:
...
**Setup (API-based):** Use API calls to create test data. This is faster and more reliable than UI-based setup. Reference `references/app-surface.md` for endpoint patterns.
...
Use the Scenario format from `references/skill-interfaces.md`:
...
1. Check `references/ptu-chapter-index.md` → "How to Look Up Base Stats" section
